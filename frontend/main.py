...
RESOURCE = os.environ.get(
    ...
    "projects/775382837439/locations/us-east1/reasoningEngines/7449158292833566720",
)
...
_A2UI_TAG_PATTERN = re.compile(r"<a2ui-json>\s*(.*?)\s*</a2ui-json>", re.DOTALL)


def _extract_parts(parts: list) -> list[dict]:
    """Turn A2A response parts into structured parts for the chat UI.

    Text parts pass through as {"kind": "text"}. If text contains <a2ui-json> tags,
    we extract the A2UI component JSON and return it as {"kind": "a2ui", "data": ...},
    stripping the raw JSON tag from the text bubble.
    ...
    for p in parts:
        ...
        if isinstance(root, TextPart) and getattr(root, "text", None):
            text = root.text
            matches = _A2UI_TAG_PATTERN.findall(text)
            if matches:
                clean_text = _A2UI_TAG_PATTERN.sub("", text).strip()
                if clean_text:
                    ...
                    try:
                        ...
                        if isinstance(parsed, dict):
                            if "components" in parsed:
                                out.append({"kind": "a2ui", "data": {"surfaceUpdate": {"components": parsed["components"]}}})
                            elif "surfaceUpdate" in parsed or "beginRendering" in parsed:
                                out.append({"kind": "a2ui", "data": parsed})
                            else:
                                out.append({"kind": "a2ui", "data": {"surfaceUpdate": {"components": [parsed]}}})
                    except Exception:
                        out.append({"kind": "text", "text": match})
            else:
